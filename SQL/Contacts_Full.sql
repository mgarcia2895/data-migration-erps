--Contact data cleaned for data migration. Selected columns are made to populate full template.
    ...
    from (
        select distinct
            ...
            ,case
                ...
                then
                    substring(
                        ...
                        position('>' in lower(arcusto.prime_contact_email)) - position('<' in lower(arcusto.prime_contact_email)) - 1
                    )
            ...
            ,row_number() over (
                ...
                order by
                    case when v.industry is not null then 1 else 2 end -- prioritize non-null market_industry
            ) as row_num
        from unique_customers
...
,bill_to_final as (
    ...
    select distinct
        ...
        -- If the number starts with "00" or "+", keep the full cleaned string
            ...
            regexp_replace(bill_to.phone_number, '[^0-9]', '') -- remove non-digit characters, keep full number
        -- For standard North American numbers, keep only the first 10 digits
            ...
            regexp_replace(bill_to.phone_number, '[^0-9]', '') -- if 10 or fewer digits, keep them all
